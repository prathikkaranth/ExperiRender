...
void VulkanEngine::save_screenshot_render_only() {
    ...
    AllocatedBuffer stagingBuffer = vkutil::create_buffer(
        this,
        finalImage.imageExtent.width * finalImage.imageExtent.height *
    ...
    immediate_submit([&](VkCommandBuffer cmd) {
        ...
        vkCmdCopyImageToBuffer(cmd, finalImage.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
    ...
    std::vector<uint8_t> ldr_data(finalImage.imageExtent.width *
                                  finalImage.imageExtent.height * 3);

    for (uint32_t i = 0;
         i < finalImage.imageExtent.width * finalImage.imageExtent.height;
         i++) {
    ...
    if (stbi_write_png(ss.str().c_str(), finalImage.imageExtent.width,
                       finalImage.imageExtent.height, 3, ldr_data.data(),
                       finalImage.imageExtent.width * 3)) {
