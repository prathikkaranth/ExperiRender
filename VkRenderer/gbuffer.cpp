...
void Gbuffer::init_gbuffer(VulkanEngine* engine) {
	...
	_gbufferPosition = vkutil::create_image(engine, VkExtent3D{ engine->_windowExtent.width, engine->_windowExtent.height, 1 }, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
	vmaSetAllocationName(engine->_allocator, _gbufferPosition.allocation, "GBuffer Position Image");
	_gbufferNormal = vkutil::create_image(engine, VkExtent3D{ engine->_windowExtent.width, engine->_windowExtent.height, 1 }, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
...
void Gbuffer::draw_gbuffer(VulkanEngine* engine, VkCommandBuffer cmd) {
	...
	_gbufferInputDescriptors = engine->globalDescriptorAllocator.allocate(engine->_device, _gbufferInputDescriptorLayout);
