...
void DescriptorLayoutBuilder::add_bindings(uint32_t binding, VkDescriptorType type, uint32_t descCount) {
    VkDescriptorSetLayoutBinding newbind{};
    newbind.binding = binding;
    newbind.descriptorCount = descCount;
    newbind.descriptorType = type;
...
void DescriptorAllocatorGrowable::init(VkDevice device, uint32_t maxSets, std::span<PoolSizeRatio> poolRatios) {
    ...
    setsPerPool = static_cast<uint32_t>(maxSets * 1.5); // grow it next allocation
...
void DescriptorWriter::write_accel_struct(int binding, VkWriteDescriptorSetAccelerationStructureKHR asInfo,
...
void DescriptorWriter::write_images(int binding, VkDescriptorImageInfo &info, VkDescriptorType type,
                                    uint32_t descCount) {
